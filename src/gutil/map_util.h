...
#include "gutil/map_traits.h"
#include "gutil/no_destructor.h"
...
template <typename M>
struct HasTryEmplace<
    M,
    absl::void_t<decltype(std::declval<M>().try_emplace(std::declval<const MapUtilKeyT<M> &>()))>>
    : std::true_type
{
};
...
struct InitType<M,
                typename std::enable_if<
                    !std::is_convertible<typename M::init_type, MapUtilValueT<M>>::value>::type>
{
...
{
    static const gutil::NoDestructor<V> value_initialized_default{};
...
template <typename M>
const MapUtilMappedT<M> &
FindWithDefault(const M &m, const MapUtilKeyT<M> &key, const MapUtilMappedT<M> &value)
{
    auto it = m.find(key);
    if (it != m.end())
        return gutil::subtle::GetMapped(*it);
...
bool FindCopy(const M &m, const Key &key, Value *value)
{
    ...
    if (it == m.end())
        return false;
    if (value)
        *value = gutil::subtle::GetMapped(*it);
    return true;
}
...
bool ContainsKeyValuePair(const M &m, const Key &key, const Value &value)
{
    ...
    for (auto it = range.first; it != range.second; ++it) {
        if (gutil::subtle::GetMapped(*it) == value) {
            return true;
        }
    }
    return false;
...
template <typename M, std::enable_if_t<internal_map_util::HasTryEmplace<M>::value, int> = 0>
bool InsertOrUpdate(M *m, const MapUtilKeyT<M> &key, const MapUtilMappedT<M> &value)
{
    ...
    subtle::GetMapped(*ret.first) = value; // update
    return false;
}
...
template <typename M, std::enable_if_t<internal_map_util::HasTryEmplace<M>::value, int> = 0>
bool InsertAndDeleteExisting(M *m, const MapUtilKeyT<M> &key, const MapUtilMappedT<M> &value)
{
    auto ret = m->try_emplace(key, value);
    if (ret.second) {
        return true;
...
template <typename M, std::enable_if_t<internal_map_util::HasTryEmplace<M>::value, int> = 0>
bool InsertIfNotPresent(M *m, const MapUtilKeyT<M> &key, const MapUtilMappedT<M> &value)
{
    return m->try_emplace(key, value).second;
}

template <typename M, std::enable_if_t<!internal_map_util::HasTryEmplace<M>::value, int> = 0>
bool InsertIfNotPresent(M *m, const MapUtilKeyT<M> &key, const MapUtilMappedT<M> &value)
{
    return m->emplace(key, value).second;
}
...
template <typename M>
bool UpdateReturnCopy(M *m, const MapUtilValueT<M> &vt, MapUtilMappedT<M> *previous)
{
    auto ret = m->insert(vt);
    if (ret.second) {
        return false;
...
MapUtilMappedT<M> EraseKeyReturnValuePtr(M *m, const MapUtilKeyT<M> &key)
{
    ...
    if (it == m->end())
        return nullptr;
    MapUtilMappedT<M> v = std::move(gutil::subtle::GetMapped(*it));
    m->erase(it);
    return v;
}
